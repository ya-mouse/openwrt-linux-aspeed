...
static char * __init unpack_to_rootfs(char *buf, int len, int check_only)
{
	int written;
	unsigned old_myinptr = -1;
	dry_run = check_only;
	...
	while (!message && len > 0) {
		...
		message = NULL; /* Zero out message, or else cpio will
				   think an error has already occured */
		if (!unxz(buf, len, NULL, flush_buffer, NULL,
			    &my_inptr, error) &&
		    message == NULL) {
			goto ok;
		}
#endif
ok:
		printk("state: %d != %d (%x) %d\n", state, Reset, my_inptr, len);
		if (state != Reset)
			error("junk in compressed archive");
		this_header = saved_offset + my_inptr;
		if (my_inptr == old_myinptr) break;
		buf += my_inptr;
		len -= my_inptr;
		old_myinptr = my_inptr;
	}
	dir_utime();
	kfree(name_buf);
	kfree(symlink_buf);
