...
static void ihold(struct inode *inode)
{
...
static int ovl_dir_fsync(struct file *file, struct dentry *d, int datasync)
{
	struct ovl_dir_file *od = file->private_data;
...
static int ovl_permission(struct inode *inode, int mask)
{
	...
	if (realinode->i_op->permission)
		...
		err = generic_permission(realinode, mask, NULL);
out_dput:
	dput(alias);
	return err;
}
