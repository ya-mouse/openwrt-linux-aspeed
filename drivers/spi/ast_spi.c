...
{
//	dev_dbg(spi->dev, "ast_spi_write : val: %x , reg : %x \n",val,spi->reg + reg);	
...
//	writel( (readl(host->spi_data->ctrl_reg) | SPI_CMD_USER_MODE) | SPI_CE_INACTIVE,host->spi_data->ctrl_reg);
	ast_spi_write(host, (ast_spi_read(host, AST_SPI_CTRL) & 0xfffffff8) | SPI_CMD_USER_MODE, AST_SPI_CTRL);
...
//	udelay(200);
	...
	list_for_each_entry(xfer, &msg->transfers, transfer_list) {
			dev_dbg(host->dev,
					"xfer[%d] %p: width %d, len %d, tx %p/%08x, rx %p/%08x\n",
					j, xfer,
...
//					if (xfer->len == 4 || xfer->len == 1)
//					dev_dbg(host->dev, "[%d] : %x @ %x\n",i, tx_buf[i], host->buff);
...
//					dev_dbg(host->dev, "[%d] : %x \n",i, rx_buf[i]);
	...
	ast_spi_write(host, (ast_spi_read(host, AST_SPI_CTRL) & 0xfffffff8) | SPI_CMD_USER_MODE | SPI_CE_INACTIVE, AST_SPI_CTRL);
//	writel( SPI_CE_INACTIVE | readl(host->spi_data->ctrl_reg),host->spi_data->ctrl_reg);
